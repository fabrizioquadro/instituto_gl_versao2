...
@section('title', 'Editar Soro - Instituto GL')

@section('content')
  <div class="card">
    <div class="card-header">
      <h5 class="mb-0">Editar Soro: {{ $soro->nome }}</h5>
    </div>
    <div class="card-body">
      @if ($errors->any())
      ...
      <form method="POST" action="{{ route('config.soros.update', $soro->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="row gy-4">
          <div class="col-md-6">
            ...
            <input type="text" class="form-control @error('nome') is-invalid @enderror" id="nome" name="nome" value="{{ old('nome', $soro->nome) }}" required />
            @error('nome')
        ...
        <div class="table-responsive">
          <table class="table">
            ...
            <tbody id="tabelas_medicamentos">
              @foreach ($soro->medicamentos as $med)
...
@section('scripts')
  ...
  <script>
    ...
    function excluir_cad(soroMedicamentoId) {
      if (!confirm('Tem certeza que deseja excluir esta linha?')) return;
      $.ajax({
        url: '{{ route('config.soros.delete_medicamento', ['soroMedicamento' => '__ID__']) }}'.replace('__ID__', soroMedicamentoId),
        method: 'DELETE',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        success: function (json) {
          if (json.controle === 'true') {
            document.getElementById('linha_cad_' + soroMedicamentoId).remove();
          } else {
