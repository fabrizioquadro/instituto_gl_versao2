...
@section('title', 'Novo Soro - Instituto GL')

@section('content')
  <div class="card">
    <div class="card-header">
      <h5 class="mb-0">Novo Soro</h5>
    </div>
    <div class="card-body">
      @if ($errors->any())
      ...
      <form method="POST" action="{{ route('config.soros.store') }}">
        @csrf
        ...
        <div class="table-responsive">
          <table class="table">
            ...
            <tbody id="tabelas_medicamentos">
              <tr id="linha_med_1">
                <td>
                  <select name="medicamento_id_1" class="form-select" required>
                ...
                <td><input type="number" min="0" step="0.01" name="quantidade_1" class="form-control" required /></td>
                <td><input type="text" inputmode="decimal" name="valor_1" class="form-control" oninput="mascaraMoeda(this)" required /></td>
                <td>
        ...
        <div class="mt-4">
          ...
          <a href="{{ route('config.soros.index') }}" class="btn btn-outline-secondary">Cancelar</a>
        </div>
      </form>
    </div>
  </div>
...
@section('scripts')
  <script>
    ...
    document.getElementById('btn_adicionar_medicamento').addEventListener('click', function () {
      ...
      tr.innerHTML = `
        <td>
          <select name="medicamento_id_${contador}" class="form-select" required>
            <option value="">Selecione</option>
            ${opcoesMedicamentos}
          </select>
        </td>
        <td><input type="number" min="0" step="0.01" name="quantidade_${contador}" class="form-control" required /></td>
        <td><input type="text" inputmode="decimal" name="valor_${contador}" class="form-control" oninput="mascaraMoeda(this)" required /></td>
        <td>
