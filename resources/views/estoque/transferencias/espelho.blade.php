...
<head>
    <meta charset="UTF-8">
    <title>Espelho de Transferência #{{ $transferencia->id }}</title>
    <style>
...
<body>
    ...
    <h1>Espelho de Transferência</h1>
    <h2>Nº {{ $transferencia->id }} — {{ $transferencia->data ? \Carbon\Carbon::parse($transferencia->data)->format('d/m/Y H:i') : '-' }}</h2>

    <div class="info">
        <div><strong>Origem:</strong> {{ $transferencia->origem?->nome ?? '-' }}</div>
        <div><strong>Destino:</strong> {{ $transferencia->destino?->nome ?? '-' }}</div>
        <div><strong>Responsável:</strong> {{ $transferencia->user?->nome ?? '-' }}</div>
        <div><strong>Motivo:</strong> {{ $transferencia->motivo ?? '-' }}</div>
    </div>
